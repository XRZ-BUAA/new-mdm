...
# 数据增强函数：给数据添加随机噪声
def gaussian_noisy_data(data, noise_factor):
...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return motion.float(), noisy_full.float(), noisy_sparse.float()
...
def load_data(dataset, dataset_path, split, **kwargs):
    ...
    motions, fulls, sparses = get_motion(motion_list)
    ...
    return new_motions, new_fulls, new_sparses, mean, std
...
def get_dataloader(
    ...
    if split == "train":
        shuffle = True
        drop_last = True    # 如果数据集的大小不是批次大小的整数倍，则丢弃最后一个不完整的批次，确保每个批次都有相同数量的数据样本
    ...
    loader = DataLoader(
        ...
        drop_last=drop_last,    # 是否丢弃最后一个不完成的数据批次
        persistent_workers=False,   # 不保持工作线程活跃
    )
    return loader
